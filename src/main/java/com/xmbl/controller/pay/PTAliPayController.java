...
public class PTAliPayController extends BaseController{
	...
	public ResponseResult ali_pay(HttpServletRequest request,String jsonData) {
        ...
        try {
        	...
        	Assert.isTrue(jsonData != null, "jsonData不能为空!");
      	  	// Step1. 解析客户端的请求的订单信息
			...
			log.info("infoMsg:================ 支付宝充值服务器返回的信息 ==================== response,{}",JSON.toJSONString(response));
	    	      
			if(response.isSuccess()){
				log.info("infoMsg:================ 支付宝预付单下单成功!,orderTime,{}",DateUtils.formatDate(new Date(), "YYYY:MM:dd HH:mm:ss"));
				body = response.getBody();
				log.info("返回客户端的body,{}=========================",body);
				// Step4. 生成支付宝充值订单
				boolean result = rechargeRecordService.generatedAliPayBills(params,Amount,outTradeNo);
				if(result) {
					log.info("infoMsg:================ 平台订单生成成功!,orderTime,{}",DateUtils.formatDate(new Date(), "YYYY:MM:dd HH:mm:ss"));
					return successJson(body);
				}
				log.info("infoMsg:================ 平台订单生成失败!,orderTime,{}",DateUtils.formatDate(new Date(), "YYYY:MM:dd HH:mm:ss"));
			} else {
	...
	public String alipay_notify(HttpServletRequest request) {
		...
		try {
			...
			log.info("infoMsg:================ 支付宝订单验证签名成功 ====================");
				// 账变，修改状态，到账提醒
    			boolean result = rechargeRecordService.updateAliPayBill(passback_params,total_amount,out_trade_no);
    			if(result){
	 ...
	 * 支付宝单笔提现
	 * 
	 * @param transferInfo {Id,PlayerId,AccountId,ServerId,Amount,PayeeType,PayeeAccount,PayeeRealName,PassWord}
...
//			String mer_key = payBean.getMer_key();	//支付宝公钥|商户私钥
			...
			if(response.isSuccess()){
				...
				if(response.getCode().equals("10000")) {
					...
					//生成账单
					String transferRemark = "方块创造于" + DateUtils.formatDate(new Date(), "YYYY-mm-dd HH:mm:ss") + ",提现失败。失败原因为：" + "支付宝账号和姓名不匹配，请确认姓名是否正确" + "提现金额为：" +Amount + 
							"元" + "流水号为：" + Id;
					boolean updateState = transferRecordService.updateBill(String.valueOf(Id), transferRemark);
					if(updateState) {
